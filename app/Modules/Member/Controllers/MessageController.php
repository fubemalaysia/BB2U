...
class MessageController extends Controller {
  ...
  public function newMessage($username = NULL) {
    ...
    if ($username != NULL) {
      ...
      if(!$messageTo){
          return redirect('messages/new-thread')->with('msgError', 'User does not found.');
      }
  ...
  public function sendMessage(Request $get) {
    ...
    if (!$user) {
      ...
      return redirect()->back()->withInput()->with('msgError', 'Your account not found');
    }
...
//      }
      if ($messageReplay->save()) {
        ...
        return redirect('messages/new-thread')->with('msgInfo', 'Message has been sent to ' . $get->username . ' ');
      }
      return redirect()->back()->withInput()->with('msgError', 'System error.');
    }
    return redirect()->back()->withInput()->with('msgError', 'Can not sent to yourself.');
  }
...
//      ->whereRaw("(r.userId <>  {$userLogin->id} && receivedId = 0) || (r.userId = {$userLogin->id} && receivedId <> {$userLogin->id})")
  ...
  public function getPrivateMessage($threadId) {
    ...
    if (!$ThreadMessage) {
      return redirect('messages')->with('msgError', 'Message thread ID not found !');
    }
    $userLogin = AppSession::getLoginData();
...
//    $this->dispatch($job);
  ...
  public function postRelayPrivateMessage(Request $get, $threadId) {
    ...
    if (empty($getMessageThread)) {
      return redirect('messages')->with('msgError', 'Conversation not found !');
    }
    $messageReplay = new MessageReplyModel();
  ...
  public function setTrashMessage($threadId, $action, $reid) {
    ...
    return redirect('messages')->with('msgError', 'Request not found !');
  }
